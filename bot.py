...
class TikTokTrackerBot(discord.Client):
    ...
    def register_commands(self):
        ...
        async def tracked(interaction: discord.Interaction):
            ...
            if not rows:
                await interaction.response.send_message(
                    "No active videos are being tracked.",
                    ephemeral=True,
                )
                return
            ...
            for row in rows:
                ...
                lines.append(
                    f'- **@{creator}** | `{views}` views | "{desc}" | [VIEW HERE]({url})'
                )
        ...
        async def archive_old_hits(interaction: discord.Interaction):
            ...
            eligible_count = await bot.count_archive_eligible_hits(
                str(interaction.guild.id)
            )
    ...
    async def send_capacity_full_warning(self, guild: discord.Guild, settings):
        channel_id = (
            settings["hit_channel_id"]
            or settings["daily_report_channel_id"]
            or settings["source_channel_id"]
        )
    ...
    async def check_guild_active_videos(self, guild_id: str) -> dict:
        ...
        for row in rows:
            ...
            try:
                ...
                errors += 1
                logger.warning(
                    "TikTok stats unavailable for %s: %s",
                    row["video_url"],
                    exc,
                )
                ...
                errors += 1
                logger.exception(
                    "Unexpected TikTok stats error for %s",
                    row["video_url"],
                )
                await asyncio.sleep(1)
                continue
    ...
    async def send_daily_report(self, guild_id: str, report_date):
        ...
        for index, row in enumerate(rows, start=1):
            ...
            lines.append(
                f'{index}. **@{creator}** | "{desc}" | [VIEW HERE]({url})'
            )
